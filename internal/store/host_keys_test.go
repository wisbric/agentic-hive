...
func TestStoreAndGetHostKey(t *testing.T) {
	...
	srv, err := s.CreateServer("hk-test", "hk.example.com", 22, "root", "", "local", "")
	if err != nil {
...
func TestGetHostKeyMissing(t *testing.T) {
	...
	srv, err := s.CreateServer("hk-missing", "hk2.example.com", 22, "root", "", "local", "")
	if err != nil {
		t.Fatalf("CreateServer failed: %v", err)
	}
...
func TestDeleteHostKey(t *testing.T) {
	...
	srv, err := s.CreateServer("hk-del", "hk3.example.com", 22, "root", "", "local", "")
	if err != nil {
		t.Fatalf("CreateServer failed: %v", err)
	}
...
func TestStoreHostKeyUpsert(t *testing.T) {
	...
	srv, err := s.CreateServer("hk-upsert", "hk4.example.com", 22, "root", "", "local", "")
	if err != nil {
		t.Fatalf("CreateServer failed: %v", err)
	}
